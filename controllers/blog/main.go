...
func (this *MainController) Index() {
	...
	if pagesize, err = strconv.Atoi(this.Ctx.Input.Param("pagesize")); err != nil || pagesize < 1 {
		pagesize = 10
	}
	offset := pagesize * (page - 1)
	query := new(models.Post).Query().Filter("status", 0).Filter("url_type", 0)
	count, _ := query.Count()
	if count > 0 {
...
func (this *MainController) Archives() {
	...
	if count > 0 {
		...
		for _, val := range list {
			year := val.PostTime.Format("2006")
			if _, ok := result[year]; !ok {
				result[year] = make([]*models.Post, 0)
			}
			result[year] = append(result[year], val)
		}
	}
	this.Data["page"] = page
	this.Data["count"] = count
...
func (this *MainController) Category() {
	...
	if count > 0 {
		...
		offset := pagesize * (page - 1)
		query.OrderBy("-post_time").Limit(pagesize, offset).All(&tp)
		for _, val := range tp {
			pids = append(pids, val.PostId)
		}
